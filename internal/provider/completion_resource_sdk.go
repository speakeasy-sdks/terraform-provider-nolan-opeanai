...
func (r *CompletionResourceModel) ToCreateSDKType() *shared.CreateCompletionRequest {
	...
	var model interface{}
	str := new(string)
	if !r.Model.Str.IsUnknown() && !r.Model.Str.IsNull() {
		*str = r.Model.Str.ValueString()
	} else {
		str = nil
	}
	if str != nil {
	...
	createCompletionRequestModel2 := new(shared.CreateCompletionRequestModel2)
	if !r.Model.CreateCompletionRequestModel2.IsUnknown() && !r.Model.CreateCompletionRequestModel2.IsNull() {
		*createCompletionRequestModel2 = shared.CreateCompletionRequestModel2(r.Model.CreateCompletionRequestModel2.ValueString())
	} else {
		createCompletionRequestModel2 = nil
	}
	if createCompletionRequestModel2 != nil {
		model = createCompletionRequestModel2
	}
	n := new(int64)
...
func (r *CompletionResourceModel) RefreshFromCreateResponse(resp *shared.CreateCompletionResponse) {
	...
	for _, choicesItem := range resp.Choices {
		...
		// Not Implemented choicesItem.Logprobs.TokenLogprobs, {"Fields":[],"BaseName":"","Input":false,"Output":false,"AdditionalProperties":null,"Name":"","Enum":null,"Scope":"","Examples":[],"Discriminator":null,"ItemType":{"Type":"number","Fields":[],"RefType":"","Truncated":false,"ItemType":null,"Input":false,"Extensions":{"x-speakeasy-param-readonly":true,"x-speakeasy-param-computed":true,"x-speakeasy-param-force-new":true,"Symbol":"TokenLogprobs"},"Format":"","Discriminator":null,"Name":"","AssociatedTypes":[],"Enum":null,"Scope":"","BaseName":"","Comments":null,"Output":false,"Examples":[],"AdditionalProperties":null},"AssociatedTypes":[],"RefType":"","Truncated":false,"Extensions":{"x-speakeasy-param-computed":true,"x-speakeasy-param-force-new":true,"x-speakeasy-param-readonly":true},"Type":"array","Comments":null,"Format":""}, true, , , choices1.Logprobs.TokenLogprobs
		...
		choices1.Logprobs.TopLogprobs = nil
		for _, topLogprobsItem := range choicesItem.Logprobs.TopLogprobs {
			var topLogprobs1 map[string]types.Int64
			if topLogprobs1 == nil && len(topLogprobsItem) > 0 {
				topLogprobs1 = make(map[string]types.Int64)
				for key, value := range topLogprobsItem {
					topLogprobs1[key] = types.Int64Value(value)
				}
			}
			choices1.Logprobs.TopLogprobs = append(choices1.Logprobs.TopLogprobs, topLogprobs1)
		}
	...
	modelResult, _ := json.Marshal(resp.Model)
	r.Model = types.StringValue(string(modelResult))
